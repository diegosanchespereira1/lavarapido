...
branchesRoutes.get("/:id", async (c) => {
    ...
    if (routeId instanceof Response)
        return routeId;
    const id = routeId;
    const forbidden = problema(c, 403, "Você não tem permissão para acessar esta filial.", CODIGOS.NAO_AUTORIZADO);
...
branchesRoutes.patch("/:id", async (c) => {
    const denied = requireAdmin(c);
    if (denied)
        return denied;
    let body;
    ...
    if (!body.name) {
        return problema(c, 400, "Informe pelo menos o campo name para atualizar.", CODIGOS.VALIDACAO);
    }
    const auth = c.var.auth;
    const routeId = requireRouteId(c);
    if (routeId instanceof Response)
