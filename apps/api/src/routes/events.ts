...
function heartbeatLoop(stream: SseStream, abortSignal?: AbortSignal) {
  ...
  if (abortSignal) abortSignal.addEventListener("abort", stop);
  return stop;
}

/** SSE só com snapshot inicial — sem polling; atualizações vêm do pub/sub. */
function snapshotOnlyStream(c: AppCtx, snapshot: () => Promise<unknown>) {
...
eventsRoutes.get("/stream", async (c: AppCtx) => {
  ...
  async function snapshot() {
    return withTenantTx(auth.tenantId, async (tx) =>
      tx
        .select()
        .from(vehicleEntries)
        .where(
          and(
            ...
            eq(vehicleEntries.branchId, branchId),
          ),
        )
