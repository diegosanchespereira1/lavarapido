...
export const cashRegisterRoutes = new Hono<{ Variables: AppVariables }>();

cashRegisterRoutes.get("/", async (c: AppCtx) => {
  ...
  if (!dateStr.success) {
    return problema(
      c,
      400,
      "Informe uma data válida em date=YYYY-MM-DD.",
      CODIGOS.VALIDACAO,
    );
  }
  ...
  if (!consolidated && !branchQuery.data) {
    return problema(
      c,
      400,
      "Envie consolidated=true ou branch_id com uma filial válida.",
      CODIGOS.VALIDACAO,
    );
  }
  ...
  if (consolidated && branchQuery.data) {
    return problema(
      c,
      400,
      "Não combine consolidated=true com branch_id. Use apenas um modo.",
      CODIGOS.VALIDACAO,
    );
  }
  ...
  if (!consolidated) {
    ...
    if (
      ...
      return problema(
        c,
        403,
        "Você não tem permissão para consultar o caixa desta filial.",
        CODIGOS.NAO_AUTORIZADO,
      );
    }
    rows = await withTenantTx(auth.tenantId, async (tx) =>
      tx
        .select({
  ...
  const total_centavos = rows.reduce((acc: number, r) => acc + Number(r.totalCents), 0);

  /** Modo detalhe por método quando há filtro de filial explícito. */
  if (!consolidated && branchQuery.data) {
    const methodRows = await withTenantTx(auth.tenantId, async (tx) =>
      tx
        .select({
