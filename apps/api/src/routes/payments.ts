...
import type { AppCtx, AppVariables } from "../lib/guards.js";
...
paymentsRoutes.post("/", async (c: AppCtx) => {
  ...
  const headerBranch =
    typeof c.var.branchId === "string" ? c.var.branchId : undefined;
  if (
    headerBranch &&
    ...
    return problema(
      c,
      400,
      "O campo branch_id deve coincidir com o cabeçalho X-Branch-Id.",
      CODIGOS.VALIDACAO,
    );
  }
  ...
  if (!branchResolved) {
    return problema(
      c,
      400,
      "Informe filial via cabeçalho X-Branch-Id ou campo branch_id no corpo.",
      CODIGOS.VALIDACAO,
    );
  }
  ...
  if (!headerBranch) {
    if (
      auth.allowedBranchIds !== "all" &&
      !auth.allowedBranchIds.includes(branchResolved)
    ) {
      return problema(
        c,
        403,
        "Você não tem permissão para registrar pagamento nesta filial.",
    ...
    if (!ok) {
      return problema(
        c,
        404,
        "Filial não encontrada neste cliente.",
        CODIGOS.NAO_ENCONTRADO,
      );
    }
  }

  let vehicleEntryPlate: string | null = null;
  ...
  if (body.vehicle_entry_id) {
    ...
    const entryCheck = await withTenantTx(auth.tenantId, async (tx) => {
      const rows = await tx
        ...
        .where(
          and(
            eq(vehicleEntries.id, vehicleEntryId),
            eq(vehicleEntries.tenantId, auth.tenantId),
            eq(vehicleEntries.branchId, branchResolved),
          ),
        )
        .limit(1);
      const entry = rows[0];
      ...
      const existing = await findPaymentForEntry(
        tx,
        auth.tenantId,
        vehicleEntryId,
      );
      ...
      if (!resolvedAmount) {
        const wtRows = await tx
          ...
          .where(
            and(
              eq(washTypes.id, entry.washTypeId),
              eq(washTypes.tenantId, auth.tenantId),
            ),
          )
          .limit(1);
        resolvedAmount = wtRows[0]?.priceCents;
        if (!resolvedAmount) return { noPrice: true as const };
      }
    ...
    if ("notFound" in entryCheck && entryCheck.notFound) {
      return problema(
        c,
        404,
        "Entrada não encontrada na filial informada.",
        CODIGOS.NAO_ENCONTRADO,
      );
    }
    if ("alreadyPaid" in entryCheck && entryCheck.alreadyPaid) {
      return problema(
        c,
        409,
        "Esta lavagem já possui pagamento registrado.",
        CODIGOS.CONFLITO,
      );
    }
    if ("noPrice" in entryCheck && entryCheck.noPrice) {
      return problema(
        c,
        400,
        "Informe amount_cents ou vincule a um tipo de lavagem com preço.",
        CODIGOS.VALIDACAO,
      );
    }

    const ok = entryCheck as {
  ...
  if (!amountCents) {
    return problema(
      c,
      400,
      "Informe amount_cents para pagamentos avulsos.",
      CODIGOS.VALIDACAO,
    );
  }
