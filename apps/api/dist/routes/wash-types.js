...
export const washTypesRoutes = new Hono();
washTypesRoutes.get("/", async (c) => {
...
washTypesRoutes.get("/:id", async (c) => {
    const auth = c.var.auth;
    ...
    const row = await withTenantTx(auth.tenantId, async (tx) => {
        const rows = await tx
            .select()
            .from(washTypes)
            .where(and(eq(washTypes.id, id), eq(washTypes.tenantId, auth.tenantId)))
            .limit(1);
        return rows[0];
    });
    if (!row) {
...
washTypesRoutes.patch("/:id", async (c) => {
    const denied = requireAdmin(c);
    if (denied)
        return denied;
    let body;
    ...
    if (body.price_cents !== undefined)
        patch.priceCents = body.price_cents;
...
washTypesRoutes.delete("/:id", async (c) => {
    const denied = requireAdmin(c);
    if (denied)
        return denied;
    const auth = c.var.auth;
...
//# sourceMappingURL=wash-types.js.map