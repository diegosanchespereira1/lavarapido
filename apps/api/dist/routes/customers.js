...
import { z } from "zod";
import { formatPlate, normalizePlateInput, validateBrazilianPlate, } from "@lava-rapido/shared";
...
import { requireAdmin, requireRouteId, } from "../lib/guards.js";
function normalizePlateForDb(pl) {
...
customersRoutes.get("/search", async (c) => {
    ...
    if (q.length < 2) {
        return problema(c, 400, "Envie pelo menos 2 caracteres em q para buscar.", CODIGOS.VALIDACAO);
    }
    const pattern = `%${q}%`;
    ...
    const rows = await withTenantTx(auth.tenantId, async (tx) => {
        const entryMatches = await tx
            ...
            .from(vehicleEntries)
            .where(and(eq(vehicleEntries.tenantId, auth.tenantId), isNotNull(vehicleEntries.customerId), normalizedPlate
            ? or(ilike(vehicleEntries.plate, platePattern), eq(vehicleEntries.plate, normalizedPlate))
            : ilike(vehicleEntries.plate, platePattern)));
        const idsFromEntries = entryMatches
            .map((r) => r.customerId)
            .filter((id) => typeof id === "string");
        const whereClause = idsFromEntries.length > 0
            ? and(eq(customers.tenantId, auth.tenantId), or(...likes, inArray(customers.id, idsFromEntries)))
...
customersRoutes.get("/", async (c) => {
    const auth = c.var.auth;
    const rows = await withTenantTx(auth.tenantId, async (tx) => tx
        .select()
        .from(customers)
...
customersRoutes.post("/", async (c) => {
    ...
    const auth = c.var.auth;
    const normalizedPlate = body.plate && validateBrazilianPlate(body.plate).normalized
...
customersRoutes.patch("/:id", async (c) => {
    ...
    if (body.document !== undefined)
        patch.document = body.document ?? null;
