...
function allowedForBranch(auth: AuthState, branchId: string): boolean {
  if (auth.allowedBranchIds === "all") return true;
...
export const branchContextMiddleware = createMiddleware(async (c, next) => {
  ...
  const present = await withTenantTx(auth.tenantId, async (tx) => {
    const rows = await tx
      ...
      .from(branches)
      .where(
        and(eq(branches.id, raw), eq(branches.tenantId, auth.tenantId)),
      )
      .limit(1);
